<template>
    <UDrawer
        ...
        <template #body>
            <div
                class="select-none w-[280px] py-3 overflow-x-hidden flex flex-col h-[calc(100vh-32px)]">
                <UAlert v-if="error" :ui="{ wrapper: 'flex items-center' }" color="error"
                    variant="soft" :description="error?.message || '加载最新评论失败，请稍后重试。'" class="mt-6">
                </UAlert>
                <div v-else-if="isLoading && !recentComments.length"
                    class="fixed inset-0 flex justify-center items-center">
                    <div class="flex flex-col justify-center items-center space-y-3">
                        <UProgress animation="swing" color="primary" size="sm"
                            class="max-w-[110px]" />
                        <div class="text-sm text-neutral-400 dark:text-neutral-600">正在加载最新评论</div>
                    </div>
                </div>
                <template v-else>
                    <UAlert v-if="!recentComments.length" :ui="{ wrapper: 'flex items-center' }"
                        color="neutral" variant="soft" description="暂无最新评论"
                    ...
                    <div v-else class="flex flex-col flex-1 min-h-0">
                        <USeparator type="dashed" class="mb-4 shrink-0">
                            <span class="text-neutral-400 dark:text-neutral-600 text-sm">最新评论（刷新间隔为
                                15 秒）</span>
                        </USeparator>
                        <div class="flex-1 min-h-0 overflow-auto">
                            <TransitionGroup name="fade-slide" tag="div">
                                <div v-for="(comment, index) in recentComments" :key="comment.id"
                                    class="mb-4 flex">
                                    ...
                                    <div
                                        ...
                                        <div
                                            class="text-neutral-700 dark:text-neutral-300 text-sm break-all">
                                            {{ comment.comment }}
                                        </div>
                                    </div>
                                </div>
