    ...
    <div class="relative flex flex-col items-center bg-white/50 dark:bg-neutral-800/50 rounded-sm">
        <NuxtLink :to="{ name: 'article-id', params: { id: talk.id } }"
            ...
            <div v-if="talk.images?.length" class="w-full flex justify-center relative">
                <NuxtImg provider="directus" :src="talk.images?.[0]?.directus_files_id" loading="eager"
                    fetchpriority="high" preload placeholder format="webp" quality="80"
                    sizes="(max-width: 768px) 100vw, 768px" class="aspect-[16/3] object-cover rounded-t-sm w-full" />
                <!-- 悬浮头像（有图片时带outline） -->
                <div
                    class="absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2 z-10 rounded-full outline-3 outline-white/50 dark:outline-neutral-800/50">
                    <SharedAvatar :src="userAvatarUrl || undefined" size="md" :alt="talk.user_created.first_name" />
                </div>
            </div>
            <!-- 无图片时仅显示头像（不带outline） -->
            ...
            <div class="px-4 flex flex-col justify-center items-center w-full"
                ...
                <div class="flex flex-col items-center mt-2">
                    ...
                    <div class="block text-sm text-neutral-400 dark:text-neutral-600 mt-2 text-center">
        ...
        <div class="w-full flex items-center justify-center p-4 gap-10">
            ...
            <div class="flex items-center gap-1 group">
                <SharedCommentCounter :content-id="talk.id" :allow-comments="talk.allow_comments" :icon-size="18" />
            </div>
            <div class="flex items-center gap-1 group">
